...
# Link cost map.  [sw1][sw2] -> cost from sw1 to sw2
linkCost = defaultdict(lambda:defaultdict(lambda:None))
...
def _calc_paths ():
  ...
  def mydump ():
    ...
    for i in range(len(sws)-1):
      for j in range(i+1, len(sws)):
        c = linkCost[sws[i]][sws[j]]
        if c: inputStr+='\tlink({0},{1},{2}),\n'.format(i+1, j+1, c)
    inputStr = inputStr[:-2]
    inputStr += '\n}\n'
    outputStr = 'Output{\n'
    for i in range(len(sws)-1):
      for j in range(i+1, len(sws)):
        outputStr += '\tbestPath({0}, {1}, {2}),\n'.format(i+1, j+1, [i+1]+[sws.index(s)+1 for s in _get_raw_path(sws[i], sws[j])]+[j+1])
    outputStr = outputStr[:-2]
    outputStr += '\n}\n'
    with open('/home/ubuntu/shortest-path-trace.txt', 'a') as f:
      f.write(inputStr)
      f.write(outputStr)
      f.write('\n')
  ...
  for k in sws:
    for j,port in adjacency[k].iteritems():
      ...
      #path_map[k][j] = (1,None)
      path_map[k][j] = (linkCost[k][j],None)
      #print 'link cost from switch {0} to switch {1}: {2}'.format(k, j, linkCost[k][j])
...
class Switch (EventMixin):
  ...
  def _handle_PacketIn (self, event):
    ...
    if ip_packet is None:
      ...
      print "Source IP:", ip_packet.srcip
      ipp = pkt.ipv4()
      if ip_packet.protocol == ipp.ICMP_PROTOCOL:
        icmp_packet = ip_packet.payload
        icmp_code = icmp_packet.code
        print icmp_code
      elif ip_packet.protocol == ipp.TCP_PROTOCOL:
        tcp_packet = ip_packet.payload
        print tcp_packet.srcport, tcp_packet.dstport
      ...
      for ll in core.openflow_discovery.adjacency:
        if ll.dpid1 == l.dpid1 and ll.dpid2 == l.dpid2:
          if flip(ll) in core.openflow_discovery.adjacency:
            ...
            # Assign random cost to each link
            c = randint(1, 10)
            linkCost[sw1][sw2] = c
            linkCost[sw2][sw1] = c
